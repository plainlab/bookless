...
PANDOC_WIN_X64="$BASE_PATH/pandoc-$VERSION-windows-x86_64.zip"
...
if [ "$#" -eq 0 ] || ([ "$PLATFORM" != "mac" ] && [ "$PLATFORM" != "win" ] && [ "$PLATFORM" != "linux" ])
then
    ...
    echo ""
    echo "USAGE:"
    echo "    ./get-pandoc.sh linux|mac|win x64|arm"
    echo ""
...
then
    ...
    mv ./pandoc.exe ../pandoc-win-$ARCH.exe
    cd ..
fi
